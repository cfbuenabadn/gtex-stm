...
with cd(args.output_dir):
    ...
    read1_pipe = args.prefix+'_read1_pipe'
    read2_pipe = args.prefix+'_read2_pipe'
    read0_pipe = args.prefix+'_read0_pipe'

    subprocess.check_call(f'mkfifo {read1_pipe} {read2_pipe} {read0_pipe}', shell=True)

    # Set gzip streams
    subprocess.check_call('gzip -'+args.gzip+f' -c < {read1_pipe} > '+fastq1+' &', shell=True)
    subprocess.check_call('gzip -'+args.gzip+f' -c < {read2_pipe} > '+fastq2+' &', shell=True)
    subprocess.check_call('gzip -'+args.gzip+f' -c < {read0_pipe} > '+fastq0+' &', shell=True)
    ...
    cmd = 'java -jar -Xmx'+str(int(args.memory))+'g '+args.jar+' SamToFastq INPUT='+args.bam_file\
        ...
        +f' VALIDATION_STRINGENCY=SILENT FASTQ={read1_pipe} SECOND_END_FASTQ={read2_pipe} UNPAIRED_FASTQ={read0_pipe}'
    if args.reference_fasta is not None:
        cmd += ' REFERENCE_SEQUENCE={}'.format(args.reference_fasta)
    subprocess.check_call(cmd, shell=True)

    # Delete named pipes
    subprocess.check_call(f'rm {read1_pipe} {read2_pipe} {read0_pipe}', shell=True)

...
output = [f"ebpmf_models/gao_models/bash_scripts/batch1.{str(n)}.sh" for n in range(0,1001)]
...
for gene, out_f in snmf_bashscript_pairs:
    ...
    with open(out_f, 'a') as f:
        out_dir_name = f'ebpmf_models/gao_models/RDS_batch1/{n}/'
        _ = f.write(f'[ -d "{out_dir_name}" ] || mkdir "{out_dir_name}"\n')
            
        _ = f.write(f'Rscript scripts/run_snmf_ad_batch1.R {gene} {n}\n')
        _ = f.write(f'touch ebpmf_models/gao_models/RDS_batch1/{n}/token.txt\n')

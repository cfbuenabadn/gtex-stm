...
main <- function(){
    ...
    SummaryStats <- fread(FileIn, nrows=Inf) %>%
        separate(snp, into=c("chrom", "pos", "A1_mol", "A2_mol"), sep="_", convert=T, remove=F) %>%
        mutate(gwas_locus = str_replace(gwas_locus, "(.+?)_(.+?)_N_N_(.+)$", "\\1_\\2_\\3"))
    ...
    for (i in seq_along(Loci)){
        ...
        #if (grepl("coronary_artery_disease", Loci)) {sub_pheno <- "(\\w+)/\\d+\\.txt$"} else {sub_pheno <- "QTLs/GTEx_10/(.+?)/(.+?)/.+$"}

        SummaryStats.filtered <- SummaryStats %>%
            filter(gwas_locus == TestLocus) %>%
            mutate(phenotype_class = phenotype_id) %>%
            #mutate(phenotype_class = str_replace(source_file, sub_pheno, "\\1")) %>%
        ...
        betas <- SummaryStats.filtered %>%
            ...
            mutate(A1_mol = toupper(A1_mol), A2_mol = toupper(A2_mol), A2 = toupper(A2), A1 = toupper(A1)) %>%
            filter((setequal(c(A1_mol, A2_mol), c(A1, A2))) | ("N" %in% c(A1, A2)) |  setequal(c(A1_mol, A2_mol), c(A2, A1))    ) %>%
            ungroup() %>%

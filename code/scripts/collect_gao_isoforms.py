...
def load_ebpmf_gene(chunk, gene_id):
    ...
    df = readRDS(f'ebpmf_models/gao_models/RDS/{chunk}/{gene_id}.rds')
...
def process_gene(gene, gtf_seq3, gtf_seq5, strand):#, eta_fh, pvals_fh, H_fh, kw_pvals_fh):
    rds = load_ebpmf_gene(chunk, gene)

    seq3_lines = process_run(rds, gene, 3, strand)
    seq5_lines = process_run(rds, gene, 5, strand)

    gtf_seq3.write(seq3_lines)
    gtf_seq5.write(seq5_lines)
    

def process_run(rds, gene, K, strand):
        
    junc_file = '/project2/mstephens/cfbuenabadn/gtex-stm/code/junctions.tab.gz'
    if f'snmf_{str(K)}' in rds['rds'].keys():
        coords = rds['rds'][f'snmf_{str(K)}']['coords']
        junctions_bed = get_junctions_bed(coords, junc_file, gene)
        coords_all = list(rds['rds']['coords'])
        coords = list(rds['rds'][f'snmf_{str(K)}']['coords'])
        isoforms = get_isoforms(rds['rds'][f'snmf_{str(K)}']['ebpmf']['EF_smooth'], junctions_bed, coords, coords_all, binary_fraction=0.5, 
...
if __name__ == '__main__':
    ...
    genes = os.listdir('ebpmf_models/gao_models/RDS/' + chunk)
    genes = [x.split('.')[0] for x in genes if x.startswith('ENSG')]
    print('total genes:')
    print(len(genes))
    print('')

    selected_genes = pd.read_csv('Annotations/gencode.v44.primary_assembly.protein_coding.genes.bed.gz', sep='\t', #'../data/selected_genes.bed', sep='\t', 
                             names = ['chrom', 'start', 'end', 'gene', 'gene_symbol', 'strand'])
    

    with open(f'ebpmf_models/gao_models/tables/tmp/snmf_3.{chunk}.gtf', 'w') as gtf_seq3:
        with open(f'ebpmf_models/gao_models/tables/tmp/snmf_5.{chunk}.gtf', 'w') as gtf_seq5:
            ...
            for gene in genes:
                if gene in genes:
                    ...
                    try:
                        process_gene(gene, gtf_seq3, gtf_seq5, strand)#, eta_fh, pvals_fh, H_fh, kw_pvals_fh)
                    except:
                        continue

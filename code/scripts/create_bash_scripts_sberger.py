...
output = [f"ebpmf_models/sberger_models/bash_scripts/{str(n)}.sh" for n in range(0,2001)]
gao_genes = list(pd.read_csv('../data/protein_coding_genes.bed.gz', sep='\t', names = ['chrom', 'start', 'end', 'gene_id', 'gene_name', 'strand']).gene_id)
...
for gene, out_f in snmf_bashscript_pairs:
    ...
    with open(out_f, 'a') as f:
        out_dir_name = f'ebpmf_models/sberger_models/RDS/{n}/'
        _ = f.write(f'[ -d "{out_dir_name}" ] || mkdir "{out_dir_name}"\n')
            
        _ = f.write(f'Rscript scripts/run_ebpmf_gregor.R {gene} {n}\n')
        _ = f.write(f'touch ebpmf_models/sberger_models/RDS/{n}/token.txt\n')

...
rule collect_coverage:
    input:
        ...
        expand("coverage/bed/Brain_Hippocampus/{IndID}.bed.bgz.tbi", IndID = brain_hippocampus_samples),
        expand("coverage/bed/Brain_Hypothalamus/{IndID}.bed.bgz.tbi", IndID = brain_hypothalamus_samples),
...
rule collect_ebpmf:
    input:
        ...
        gene=["PKM", "AR", "PBXO44"] + quick_test_genes, #genes, 
        K = [2, 3, 4, 5]),
        expand("ebpmf_model/train_10tissues/plots/{geneName}.K{kfactors}.factors.pdf",
        geneName=genes,#["PKM", "AR", "PBXO44"] + quick_test_genes, #genes, 
        kfactors = [2, 3, 4, 5]),
        
        
rule GetManifest:
    input:
        expand("gtex-download/{Tissue}/files/tissue-manifest.json", Tissue=tissue_list)